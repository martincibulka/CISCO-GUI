...
export async function openDb() {
  if (!dbInstance) {
    ...
    if (roleCount.cnt === 0) {
      await dbInstance.run("INSERT INTO app_roles (name, edit_users, edit_switches) VALUES ('admin', 1, 1)");
      await dbInstance.run("INSERT INTO app_roles (name, edit_users, edit_switches) VALUES ('používateľ', 0, 0)");
    } else {
      await dbInstance.run("UPDATE app_roles SET edit_users = 1, edit_switches = 1 WHERE name = 'admin'");
    }
  }
  return dbInstance;
}
