...
export async function openDb() {
  if (!dbInstance) {
    ...
    await dbInstance.exec(`
      CREATE TABLE IF NOT EXISTS port_change_log (
        id         INTEGER PRIMARY KEY AUTOINCREMENT,
        switch_id  INTEGER NOT NULL,
        port_name  TEXT NOT NULL,
        field      TEXT NOT NULL,
    ...
    try { await dbInstance.exec("ALTER TABLE app_roles ADD COLUMN view_logs INTEGER DEFAULT 0"); } catch(e){}

    const roleCount = await dbInstance.get('SELECT COUNT(*) as cnt FROM app_roles');
    if (roleCount.cnt === 0) {
      await dbInstance.run("INSERT INTO app_roles (name, edit_users, edit_switches, edit_roles, view_logs) VALUES ('admin', 1, 1, 1, 1)");
      await dbInstance.run("INSERT INTO app_roles (name, edit_users, edit_switches, edit_roles, view_logs) VALUES ('používateľ', 0, 0, 0, 0)");
    } else {
      await dbInstance.run("UPDATE app_roles SET edit_users = 1, edit_switches = 1, edit_roles = 1, view_logs = 1 WHERE name = 'admin'");
    }
  }
  return dbInstance;
}
